...
public class EraserObject extends CustomerShapeView {
    public static final Parcelable.Creator<EraserObject> CREATOR = new Parcelable.Creator<EraserObject>() {
        public EraserObject createFromParcel(Parcel in) {
            EraserObject mFreeStyleObject = new EraserObject();
            try {
        ...
        public EraserObject[] newArray(int size) {
            return new EraserObject[size];
        }
    };
    ...
    public EraserObject(){}
    public EraserObject(Context mContext, int mType, float x, float y, float w, float h, Path mPath, Paint mPaint, FlaotPoint mOldFlaotPointMarginBitmap, float mWdithBitamp, float mHeighBitmap) {
        ...
        this.mPaintFocus.setColor(AnnotationInitialize.getInstance().getContext().getResources().getColor(R.color.default_read));
    ...
    public EraserObject copyCustomerView() {
        EraserObject freeStyle = new EraserObject(this.mContext, this.mType, this.x, this.y, this.w, this.h, this.mPathFreeStyle, this.mPaint, this.mOldFlaotPointMarginBitmap.copyFlaotPoint(), this.mWdithBitamp, this.mHeighBitmap);
    ...
    public void scaleAndChangePosition(FlaotPoint oldBitmapMarin, FlaotPoint newBitmapMargin, float scale) {
        ...
        if (this.arrayPointPathFirstCreate != null && this.arrayPointPathFirstCreate.length > 0) {
            this.mPathFreeStyle.reset();
            for (i = 0; i < this.arrayPointPathFirstCreate.length; i++) {
    ...
    public void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        canvas.drawPath(this.mPathFreeStyle, mPaint);
        this.mPathFreeStyle.computeBounds(this.RectFBodyPath, true);
//        if (this.isFocus) {
